...
class mass_flux(object):
    ...
    def __init__(self, locs, bed, tracks, vels, file_path=None, vid_info=None):
        self.info = vid_info
    ...
    def calculate(self, frange=None, batch_size=None):
        ...
        bed_xf = self.bed.get().groupby('x').mean()
        self.y_mark = (np.sin(self.theta) * bed_xf.x.values + np.cos(self.theta) * bed_xf.y_pix_bed_lin.values).max().astype(np.int) + 10
